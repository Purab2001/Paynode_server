...
const getWorksheetsByEmployee = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Failed to fetch worksheets", error: error.message });
  }
};
...
const createWorksheet = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Failed to create worksheet", error: error.message });
  }
};
...
const updateWorksheet = async (req, res) => {
  try {
    ...
    const result = await getDB().collection("worksheets").updateOne(
      { _id: new ObjectId(id) },
      updateDoc
    );
    ...
    res.status(500).json({ success: false, message: "Failed to update worksheet", error: error.message });
  }
};
...
const deleteWorksheet = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await getDB().collection("worksheets").deleteOne({ _id: new ObjectId(id) });
    if (result.deletedCount === 0) {
      return res.status(404).json({ success: false, message: "Worksheet not found" });
    }
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ success: false, message: "Failed to delete worksheet", error: error.message });
  }
};
