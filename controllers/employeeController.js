...
async function getEmployeeStats(req, res) {
  try {
    ...
    console.log("[getEmployeeStats] All worksheet docs for", email, allDocs);

    // Hours and entries this month
    const now = new Date();
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    const hoursAgg = await worksheetCol.aggregate([
      { $addFields: { dateObj: { $toDate: "$date" } } },
      { $match: { employeeEmail: email, dateObj: { $gte: monthStart } } },
      { $group: { _id: null, totalHours: { $sum: "$hoursWorked" }, count: { $sum: 1 } } }
    ]).toArray();
    console.log("[getEmployeeStats] hoursAgg", hoursAgg);
    const hoursThisMonth = hoursAgg[0]?.totalHours || 0;
    const entriesThisMonth = hoursAgg[0]?.count || 0;

    // Last payment
    const lastPayment = await paymentCol.find({ employeeEmail: email }).sort({ date: -1 }).limit(1).toArray();
    console.log("[getEmployeeStats] lastPayment", lastPayment);
    const lastPaymentAmount = lastPayment[0]?.amount || null;
    const lastPaymentDate = lastPayment[0]?.date ? new Date(lastPayment[0].date).toLocaleDateString() : null;

    // Pending tasks (example: entries with status "pending")
    const pendingTasks = await worksheetCol.countDocuments({ employeeEmail: email, status: "pending" });

    console.log("[getEmployeeStats] Response", { hoursThisMonth, entriesThisMonth, lastPaymentAmount, lastPaymentDate, pendingTasks });
    res.json({ hoursThisMonth, entriesThisMonth, lastPaymentAmount, lastPaymentDate, pendingTasks });
  } catch (err) {
...
async function getEmployeeRecentActivity(req, res) {
  try {
    ...
    // Recent work entries
    const recentWorks = await worksheetCol.find({ employeeEmail: email }).sort({ date: -1 }).limit(5).toArray();
    // Recent payments
    const recentPays = await paymentCol.find({ employeeEmail: email }).sort({ date: -1 }).limit(2).toArray();
...
async function getEmployeeWorkSummary(req, res) {
  try {
    ...
    const weekAgg = await worksheetCol.aggregate([
      { $addFields: { dateObj: { $toDate: "$date" } } },
    ...
    // This month
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    const monthAgg = await worksheetCol.aggregate([
      { $addFields: { dateObj: { $toDate: "$date" } } },
      { $match: { employeeEmail: email, dateObj: { $gte: monthStart } } },
      { $group: { _id: null, totalHours: { $sum: "$hoursWorked" }, count: { $sum: 1 } } }
    ]).toArray();
    ...
    const efficiency = tasksCompleted ? Math.round((tasksCompleted / 20) * 100) + "%" : "N/A";
...
async function getEmployeeRecentWork(req, res) {
  try {
    const email = req.params.email;
    const worksheetCol = getCollection("worksheets");
    const recent = await worksheetCol.aggregate([
    ...
    const result = recent.map(w => ({
      ...
      date: w.dateObj ? new Date(w.dateObj).toLocaleDateString() : (w.date ? new Date(w.date).toLocaleDateString() : "")
    }));
    res.json(result);
  } catch (err) {
